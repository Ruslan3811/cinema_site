...
class Category(models.Model):
    #если поле в name не будет указано, оно заполнится как "Категория"
    name = models.CharField("Категория", max_length=200)
    # если поле в name не будет указано, оно заполнится как "Описание"
    description = models.TextField("Описание")
    #slugfield - поле, содержащее цифры, буквы, подчеркивания и дефисы
    url = models.SlugField(max_length = 160, unique=True)

    def __str__(self):
        return self.name

#Контейнер класса с некоторыми опциями метаданных, прикрепленные к модели.
#Он определяет является ли модель абстрактной, имя связанной базы данных, единственное и мн число имени и тд
    class Meta:
